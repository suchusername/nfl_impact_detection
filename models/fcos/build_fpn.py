...
def residual_feature_extraction(img_input, kwargs, reg):
...
def feature_pyramid_network(inp):
    outputs = []
    outputs.append(inp[-1])
    x = tf.keras.layers.UpSampling2D(2, interpolation="nearest")(inp[-1])
    adaptive_layer = tf.keras.layers.Conv2D(128, (1,1))(inp[-3])
    add_layer = tf.keras.layers.Add()([x, adaptive_layer])
    output = tf.keras.layers.Conv2D(128, (3,3))(add_layer)
    outputs.append(output)
    return outputs

def head_model(feature_map, kwargs):
...
def build_model(**kwargs):
    ...
    res_outputs = residual_feature_extraction(img_input, kwargs, reg)
    fpn_outputs = feature_pyramid_network(res_outputs)
    outputs = []
    for output in fpn_outputs:
        outputs.append(head_model(output, kwargs))
